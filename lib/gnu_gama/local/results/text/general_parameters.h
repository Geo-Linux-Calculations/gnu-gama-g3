...
bool GeneralParameters(GNU_gama::local::LocalNetwork* IS, OutStream& out)
{
...

    {
        ...
        {
            int n;
            n = Utf8::length(T_GaMa_gpar1_coordinates);
            if (n > w1) w1 = n;
            n = Utf8::length(T_GaMa_gpar1_adjusted_coordinates);
            if (n > w1) w1 = n;
            n = Utf8::length(T_GaMa_gpar1_constrained_coordinates);
            if (n > w1) w1 = n;
            n = Utf8::length(T_GaMa_gpar1_fixed_coordinates);
            if (n > w1) w1 = n;
            n = Utf8::length(T_GaMa_gpar1_total);
            if (n > w1) w1 = n;
        }
    ...
    {
        int n;
        n = Utf8::length(T_GaMa_gpar1_directions);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_angles);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_distances);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_observed_coords);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_leveling_diffs);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_z_angles);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_s_dists);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_obs_total);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_equations);
        if (n > w1) w1 = n;
        n = Utf8::length(T_GaMa_gpar1_redundancy);
        if (n > w1) w1 = n;
    }
    ...
    int w2 = 0;
    {
        ...
        if (pocosn)
        {
            n = Utf8::length(T_GaMa_gpar2_bearings);
            if (n > w2) w2 = n;
        }
        n = Utf8::length(T_GaMa_gpar2_number_of_unknowns);
        if (n > w2) w2 = n;
        n = Utf8::length(T_GaMa_gpar2_network_defect);
        if (n > w2) w2 = n;
    }
    const char* tab_sep = "            ";
        ...
        //if (IS->m_0_aposteriori())
        {
            ...
            if (itd+its+itu > 1)
            {
                ...
                if (its+itu)
                {
                    ...
                    {
                    case 1:
                        out << T_GaMa_m0_directions;
                        break;
                    case 2:
                        out << T_GaMa_m0_angles;
                        break;
                    case 3:
                        out << T_GaMa_m0_dirs_angs;
                        break;
                    }
                    out << m0s/ma;
                }
