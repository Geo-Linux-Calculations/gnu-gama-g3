...
public:
    ...
    void set_init_value(double p)
    {
        val = p;
        cor = 0;
    }
    void set_correction(double p)
    {
        cor = p;
    }
    void set_step_size (double p)
    {
        dif = p;
    }
    void set_index(std::size_t t)
    {
        ind = t;
    }

    void add_correction(double p)
    {
        cor += p/scale();
    }
    ...
    bool constr() const
    {
        return state_ == constr_;
    }

    void set_state(const Parameter& p)
    {
        state_ = p.state_;
    }
    bool cmp_state(const Parameter& p) const
    {
